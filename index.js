...
function cumlelereDonustur(array, ayrac = ",") {
  const yeniDizi = array.map((cumle) => cumle.join(ayrac));
  return yeniDizi;
}
...
function paragrafOlustur(array, callback, callback2) {
  const dizi = [...callback2(array, " ")];
  return callback(dizi[1], dizi[3], dizi[5], dizi[7], dizi[9]);
}
...
//or var manav = [...meyveler, ...sebzeler];
...
function emojileriDonustur(mesaj, obje) {
  for (let sembol in obje) {
    if (obje.hasOwnProperty(sembol)) {
      let kucuk = sembol.toLowerCase();
      mesaj = mesaj.replaceAll(kucuk, obje[sembol]);

      let buyuk = sembol.toUpperCase();
      mesaj = mesaj.replaceAll(buyuk, obje[sembol]);
    }
  }
  return mesaj;
}
